...
static std::string config_file = "/home/sampson/learn_ws/src/LIO-mapping-Noted/config/indoor_test_config.yaml";
...
int main(int argc, char **argv) {
  std::string Newrizon_Log_Path = "";
  if (!std::getenv("ADIT_LOG_PATH")) {
    std::cout << "HDMAP CANT Find ADIT LOG PATH! " << std::endl;
    std::cout << "Please SET ADIT LOG PATH ENV Params" << std::endl;
    std::cout << "Program Exit !" << std::endl;
    return 0;
  } else {
    Newrizon_Log_Path =
      std::string(std::getenv("ADIT_LOG_PATH")) + "/MAP/HDMAP/";
    if (!boost::filesystem::is_directory(Newrizon_Log_Path)) {
      bool bRet = boost::filesystem::create_directories(Newrizon_Log_Path);
      if (false == bRet) {
        std::cout << "CANT Create HDMAP LOG PATH, Exited" << std::endl;
        return 0;
      }
    }
    Newrizon_Log_Path += "estimator_node_";
  }

  FLAGS_stop_logging_if_full_disk = true;
  google::SetLogDestination(google::GLOG_INFO, Newrizon_Log_Path.c_str());
  google::InitGoogleLogging("estimate_node");
  ...
  nh_ptr->param("config_file", config_file, std::string("/home/sampson/learn_ws/src/LIO-mapping-Noted/config/indoor_test_config.yaml"));
