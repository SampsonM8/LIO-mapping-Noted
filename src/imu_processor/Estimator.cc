...
void Estimator::ProcessImu(double dt,
  ...
  if (!first_imu_) {   // 初始化 
    first_imu_ = true;
    acc_last_ = linear_acceleration; // 线加速度
    gyr_last_ = angular_velocity;    // 角速度
    dt_buf_.push(vector<double>());  // 第一帧并未放到容器里
    linear_acceleration_buf_.push(vector<Vector3d>());
    angular_velocity_buf_.push(vector<Vector3d>());

    Eigen::Matrix3d I3x3; // p,v,q,ba,bg 容器
    I3x3.setIdentity();
...
//  }
  ...
  if (cir_buf_count_ != 0) { // 根据变量名字也不知道是哪个buffer
    ...
    imu_stampedtransforms.push(imu_tt);                           // 更新完的数据存到新的容器里
//    DLOG(INFO) << imu_tt.transform;
  }
  acc_last_ = linear_acceleration; // 更新上一帧观测量
  gyr_last_ = angular_velocity;

  if (stage_flag_ == INITED) { // 是否初始化了，初始化了就发odom数据 // 这部分对应论文里的state prediction 框图
...
void Estimator::ProcessEstimation() {
  ...
  while (true) {
    ...
    std::unique_lock<std::mutex> buf_lk(buf_mutex_);  // 锁
    con_.wait(buf_lk, [&] {                           // 等待测量数据到来 ， 如果没数据的时候，CPU分资源给其他线程（上下文切换），上下文切换也是需要开销的。
      return (measurements = GetMeasurements()).size() != 0; // 多了数据的复制拷贝
    });
    buf_lk.unlock();     // 没必要
...
//              << measurements.front().first.size();
    ...
    for (auto &measurement : measurements) {
      ...
      TicToc tic_toc_imu;
      tic_toc_imu.Tic();
      for (auto &imu_msg : measurement.first) { // 遍历imu数据
        double imu_time = imu_msg->header.stamp.toSec(); // imu 时间戳
        double laser_odom_time = compact_data_msg->header.stamp.toSec() + mm_config_.msg_time_delay; // lidar 时间 加上一定的时间偏移 (lidar 的时间戳可能是第一个点的时间？ 或者就是lidar的时间戳不是很准？)
        if (imu_time <= laser_odom_time) {  // imu 数据比lidar 数据早一点

          if (curr_time_ < 0) {  // 第一次初始化全局时间戳
            curr_time_ = imu_time;
          }

          double dt = imu_time - curr_time_;  // 算下时间dt
          ROS_ASSERT(dt >= 0);
          curr_time_ = imu_time; // 更新全局时间 imu based
          ...
          ProcessImu(dt, Vector3d(ax, ay, az), Vector3d(rx, ry, rz), imu_msg->header);  // 处理imu数据 ，此处函数的封装感觉不是太好。

        } else {  // imu 数据比lidar 数据要新一点

          // NOTE: interpolate imu measurement
          double dt_1 = laser_odom_time - curr_time_;  // 和上次imu时间比较
          double dt_2 = imu_time - laser_odom_time; // 新imu数据和lidar数据的差
          curr_time_ = laser_odom_time; // 更新全局时间
          ROS_ASSERT(dt_1 >= 0);
          ROS_ASSERT(dt_2 >= 0);
          ROS_ASSERT(dt_1 + dt_2 > 0);
          double w1 = dt_2 / (dt_1 + dt_2);  // 线性比例
          double w2 = dt_1 / (dt_1 + dt_2); 
          ax = w1 * ax + w2 * imu_msg->linear_acceleration.x;  // 线性插值
          ay = w1 * ay + w2 * imu_msg->linear_acceleration.y;
          az = w1 * az + w2 * imu_msg->linear_acceleration.z;
          rx = w1 * rx + w2 * imu_msg->angular_velocity.x;
          ry = w1 * ry + w2 * imu_msg->angular_velocity.y;
          rz = w1 * rz + w2 * imu_msg->angular_velocity.z;
          ProcessImu(dt_1, Vector3d(ax, ay, az), Vector3d(rx, ry, rz), imu_msg->header); // 继续处理imu数据
